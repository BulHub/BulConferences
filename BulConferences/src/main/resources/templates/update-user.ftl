...
<div class="site-wrap">
    ...
    <form id="form3" autocomplete="off" method="post" action="/admin/update">
    ...
    <script>
        function validateForm4() {
            ...
            let error = '';
            if (nickname.value.length === 0 && password1.value.length === 0){
                error+='None of the fields are filled in to update the user \n';
            }
            ...
            if ((nickname.value.length > 6 && nickname.value.length < 30)
                || (password1.value.length > 6 && password1.value.length < 30)){
                error='';
            }
